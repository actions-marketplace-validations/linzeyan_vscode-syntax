...
```sh
code --install-extension poly-syntax-highlight-0.11.0.vsix
code --install-extension poly-lsp-darwin-arm64-0.11.0.vsix
code --install-extension poly-editor-0.11.0.vsix
```
...
> poly-syntax-0.11.0.vsix). The VSIX files were downloaded — install them
...
```sh
POLY_VERSION=0.11.0 POLY_INSTALL_DIR=~/bin sh install.sh
```
...
`@v0` 會跟著最新的 release 走。要釘死版本就寫 `with: { version: "0.11.0" }`——poly
...
`linux/amd64` 與 `linux/arm64` 都有。tag 有 `latest`、`0.11.0`、`0.11`；pre-release
